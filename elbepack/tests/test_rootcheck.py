...
def test_check_rootful_requirements_raises_for_cdrom_param_when_loop_unavailable(
        tmp_path, monkeypatch):
    monkeypatch.setattr('elbepack.rootcheck.loop_mount_available', lambda: False)
    xmlfile = tmp_path / 'x.xml'
    xmlfile.write_text('<xml><target/></xml>')
    with pytest.raises(CliError) as exc_info:
        check_rootful_requirements(str(xmlfile), cdrom='/some.iso')
    assert 'loop-mounted' in str(exc_info.value)
