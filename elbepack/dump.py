...
def _read_apt_sources(root=pathlib.Path('/')):
    ...
    if sources_dir.exists():
        for path in [
                *sorted(sources_dir.glob('*.list')),
                *sorted(sources_dir.glob('*.sources')),
        ]:
            if path.is_file():
...
def elbe_report(xml, buildenv, cache, targetfs, builddir):
    ...
    slist = _read_apt_sources(pathlib.Path(rfs.path))
    report.info('')
