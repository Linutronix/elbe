...
def _loop_mount_reasons(xml, cdrom=None):
    reasons = []

    if cdrom is not None or xml.has('project/mirror/cdrom'):
        reasons.append(
            'the build installs packages from a CD-ROM/ISO mirror, which is '
            'loop-mounted')
    ...
    for bylabel in xml.all('target/fstab/bylabel'):
        if (bylabel.has('fs/fs-finetuning/device-command')
            ...
            reasons.append(
                ...
                'with a device-command/path-command, which loop-mounts '
                'that partition image')
        elif bylabel.has('fs/tune2fs'):
            reasons.append(
                f"filesystem '{bylabel.text('label')}' uses "
                '<tune2fs>, which loop-mounts that partition image')
...
def xml_needs_rootful(xml, cdrom=None):
    return _loop_mount_reasons(xml, cdrom) + _mknod_reasons(xml)
...
def check_rootful_requirements(xmlfile, cdrom=None):
    xml = etree(xmlfile)
    loop_reasons = _loop_mount_reasons(xml, cdrom)
    ...
    raise CliError(message=textwrap.dedent(f"""
        ...
        See the documentation for the elbe build command.
        """))
