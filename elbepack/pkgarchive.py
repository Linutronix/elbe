...
def checkout_binpkg_archive (ep, repodir):
    ...
    try:
        ...
        # We need to update the APT cache to apply the changed package source
        ...
        if path.isdir( sources_list_d_backup ) or path.isfile( sources_list_backup ):
        ...
        if path.isdir( pkgarchive ):
            ep.log.printo( "Removing package archive from build environment" )
            rmtree( pkgarchive )
