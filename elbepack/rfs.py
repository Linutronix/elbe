    ...
    def seed_etc( self ):
        ...
        if serial_baud:
            if (self.xml.text( "project/suite" ) == "wheezy") or (self.xml.text( "project/suite" ) == "squeeze"):
                ...
                self.log.chroot(self.rfs.path, """/bin/sh -c 'ln -s /lib/systemd/system/serial-getty@.service /etc/systemd/system/getty.target.wants/serial-getty@%s.service'""" % serial_con, allow_fail=True )
        else:
