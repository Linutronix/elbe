    ...
    def debootstrap (self):
        ...
        if not self.xml.is_cross (host_arch):
            if self.xml.has("project/noauth"):
                cmd = 'debootstrap --include=apt-listchanges,apt-transport-https,ca-certificates --no-check-gpg --arch=%s "%s" "%s" "%s"' % (
                            arch, suite, self.rfs.path, primary_mirror)
            else:
                cmd = 'debootstrap --include=apt-listchanges,apt-transport-https,ca-certificates --arch=%s "%s" "%s" "%s"' % (
                            arch, suite, self.rfs.path, primary_mirror)
        ...
        if self.xml.has("project/noauth"):
            cmd = 'debootstrap --include=apt-transport-https,ca-certificates --no-check-gpg --foreign --arch=%s "%s" "%s" "%s"' % (
                arch, suite, self.rfs.path, primary_mirror)
        else:
            cmd = 'debootstrap --include=apt-transport-https,ca-certificates --foreign --arch=%s "%s" "%s" "%s"' % (
                arch, suite, self.rfs.path, primary_mirror)
