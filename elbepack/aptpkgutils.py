...
def _apt_uid(passwd_root):
    passwd = os.path.join(passwd_root, 'etc/passwd')
    ...
    raise LookupError(f"'_apt' user not found in {passwd}")
...
def make_writable_by_apt(path, passwd_root='/'):
    ...
    os.chown(path, _apt_uid(passwd_root), 0)
    os.chmod(path, 0o700)
