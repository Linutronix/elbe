...
def _test_generated_elbe_files(build_dir, root):
    ...
    assert elbe_version.startswith('{} 1.0\nthis RFS was generated by elbe'.format(
...
def _test_rootfs(build_dir, root):
    image_properties = image_parameters(build_dir)
    base_image = image_properties['base-image']
    has_local_gpio = image_properties['has-local-gpio']
    has_finetuning = image_properties['has-finetuning']
    ...
    if not base_image:
        ...
        assert root.joinpath('etc', 'shadow').read_text().startswith('root:!:')
    ...
    if not base_image:
        ...
        getty_service = root.joinpath('etc', 'systemd', 'system', 'getty.target.wants',
                                      'getty@tty1.service')
        assert getty_service.is_symlink()
        assert str(getty_service.readlink()) == '/lib/systemd/system/getty@.service'
...
def test_image(build_dir):
    base_image = image_parameters(build_dir)['base-image']
    if base_image:
        with elbevalidate.Tar.from_file(build_dir / 'base-rootfs.tgz') as tar:
