...
def get_image_configuration(build_dir):
    ...
    return ImageConfiguration(
        name=project_name,
        disk_images=[disk_image] if disk_image else [],
...
def _test_generated_elbe_files(build_dir, root, image_config):
    elbe_version = root.joinpath('etc', 'elbe_version').read_text()
    assert elbe_version.startswith(f'{image_config.name} 1.0\nthis RFS was generated by elbe')
...
def _test_finetuning(root, image_config):
    ...
    # Extended images append to the original /etc/hosts
...
def _test_rootfs(build_dir, root, image_config):
...
def _test_rfs_partition(build_dir, img, part, image_config):
    ...
    with part.files() as root:
        ...
        _test_rootfs(build_dir, root, image_config)
...
def test_image(build_dir):
    ...
    for image_name in image_config.disk_images:
        with elbevalidate.Image.from_file(build_dir / image_name) as img:
            ...
            _test_rfs_partition(build_dir, img, partitions[0], image_config)
